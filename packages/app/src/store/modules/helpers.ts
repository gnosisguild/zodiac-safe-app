...
export function isRealityModule(module: Module): module is RealityModule {
  return module.type === ModuleType.REALITY_ETH || module.type === ModuleType.REALITY_ERC20
}
...
export async function fetchDelayModule(
  ...
  const delayModAddress = await delayModule.getAddress()
  try {
    const moduleContract = new MultiCallContract(delayModAddress, abi as JsonFragment[])
...
export function getTransactionsFromSafeTransaction(
  ...
  if (safeTransaction.dataDecoded && isMultiSendDataEncoded(safeTransaction.dataDecoded)) {
...
function decodeProxyFactoryTransaction(data: string) {
  return new Interface(MODULE_PROXY_FACTORY_ABI).decodeFunctionData('deployModule', data)
}
 ...
 */
export function isRemoveModuleTransactionPending(transaction: DecodedTransaction): boolean {
...
export function getModulesToBeRemoved(
  ...
  return transactions
    ...
    .map((transaction) => {
      const param = transaction.dataDecoded.parameters.find((param) => param.name === 'module')
...
export function getPendingModulesToEnable(
  ...
  return transactions
    ...
    .map((transaction): PendingModule => {
      ...
      const moduleType = modulesTypesByContractAddress[moduleAddress] || ModuleType.UNKNOWN
      return {
        address: moduleAddress,
        executor: transaction.to,
        module: moduleType,
...
export function isPendingModule(module: Module, pendingModule: PendingModule) {
  return pendingModule.address === module.address && pendingModule.executor === module.parentModule
}
