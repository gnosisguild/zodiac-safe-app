...
export const modulesMiddleware: Middleware = (store) => (next) => (action: any) => {
  if (action.type === fetchModulesList.fulfilled.type) {
    ...
    if (currentPending) {
      ...
      const currentModule = allModules.find((module) => module.address === currentPending.address)
      if (currentModule) {
  ...
  if (action.type === fetchPendingModules.fulfilled.type) {
    ...
    if (getModuleAdded(oldState)) {
      ...
      const newPendingModules = getPendingCreateModuleTransactions(store.getState())
      if (
