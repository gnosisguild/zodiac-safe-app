...
const defaultDescription = (
  <Typography variant="body2">This will add a timedelay to any transactions created by this module.</Typography>
)
  ...
  return (
    <Row style={{ alignItems: "flex-start" }}>
      ...
      <div className={classes.container}>
        <Typography gutterBottom>Attach to {type.replace(/^\w/, (c) => c.toUpperCase())} Module</Typography>
        {description}

        {checked
          ? modules.map((module) => (
              <Row key={module.address} className={classes.item}>
                <Radio checked={value === module.address} onClick={() => onChange(module.address)} />
                <Column style={{ justifyContent: "center" }}>
                  {isDelayModule(module) ? (
                    <Badge className={classNames(classes.text, classes.delayText)}>
