...
export const OracleSection: React.FC<SectionProps> = ({ handleBack, handleNext, setupData }) => {
  ...
  const [instanceData, setInstanceData] = useState<OracleInstanceData>({
    ...
    instanceType: options[0].value.substr(0, options[0].value.indexOf("-")) as "ETH" | "GNO" | "custom",
  })
      ...
      <Grid container spacing={4} className={classes.container}>
        <Grid item>
          <Grid container spacing={1} className={classes.container}>
            ...
            <Grid item>
              <Typography>
                Now, it&apos;s time to set up the oracle for your reality module. The oracle ensures the results of
                proposals are brought accurately on-chain. The Reality.eth oracle uses a mechanism known as the{" "}
        ...
        <Grid item>
          <OracleTemplate data={templateData} setData={setTemplateData} ensName={setupData?.proposal.ensName} />
        </Grid>
        ...
        <Grid item>
          <Grid container spacing={3} justifyContent="center" alignItems="center">
            ...
            <Grid item>
              <Button color="secondary" size="medium" variant="contained" onClick={() => handleNext(collectData())}>
                Next
              </Button>
            </Grid>
          </Grid>
